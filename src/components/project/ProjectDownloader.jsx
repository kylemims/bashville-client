...
export const ProjectDownloader = ({ project, files, setupInstructions, onDownloadComplete }) => {
  const [isGenerating, setIsGenerating] = useState(false);
  const [copiedStep, setCopiedStep] = useState(null);

  const handleCopyStep = async (step, index) => {
    try {
      await copyToClipboard(step);
      setCopiedStep(index);
      setTimeout(() => setCopiedStep(null), 2000); // Clear feedback after 2 seconds
    } catch (error) {
            ...
            {setupInstructions.steps.map((step, index) => (
              <div key={index} className="setup-step">
                ...
                <ActionButton
                  ...
                  {copiedStep === index ? (
                    <MaterialIcon icon="check" size={14} color="var(--color-accent)" />
                  ) : (
                    <MaterialIcon icon="content_copy" size={14} color="var(--muted)" />
                  )}
                </ActionButton>
              </div>
            ))}
